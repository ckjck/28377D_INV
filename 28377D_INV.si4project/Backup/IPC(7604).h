...
class	Class_IPC
	{
	private:
		UINT16		IPC_DataWrite(const UINT32 &ulFlag, UINT16 * ulAddress, UINT16 * ulData);
		UINT16		IPC_DataRead(const UINT32 &ulFlag, UINT16 * ulAddress, UINT16 * ulData);
 		
		union		IPC_Data		m_un_CPU1toCPU2;
	 	union		IPC_Data		m_un_CPU2toCPU1;
		
	public:
		void		Drv_IPC_ADSample(void);
